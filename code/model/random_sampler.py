...
class ErrorBoundSampler(RaySampler):
    ...
    def get_z_vals(self, ray_dirs, cam_loc, model):
        ...
        ret_dict_1 = self.Algorithm(cam_loc_list[-1], ray_dirs_list[-1], model, z_vals)
        # 第一次入射, 改变方向
        ...
        ret_dict_2 = self.Algorithm(cam_loc_list[-1], ray_dirs_list[-1], model, z_vals)
        # 第二次入射, 改变方向
        intersection = rend_util.get_points(cam_loc=cam_loc_list[-1], ray_dirs=ray_dirs_list[-1], z_vals=z_intersection)
        ...
        ret_dict_3 = self.Algorithm(cam_loc_list[-1][mask], ray_dirs_list[-1][mask.reshape(-1)], model, z_vals)
    ...
    def Algorithm(self, cam_loc, ray_dirs, model, z_vals):
        z_samples = z_vals
